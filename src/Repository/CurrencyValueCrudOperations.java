...
public interface CurrencyValueCrudOperations {
    ...
    default void addCurrencyValue(CurrencyValue currencyValue) {
        try (Connection connection = DBConnection.getConnection()) {
            String query = "INSERT INTO CurrencyValue (currency_source, currency_destination, amount, date_effect) VALUES (?, ?, ?, ?)";
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                preparedStatement.setInt(1, Integer.parseInt(currencyValue.getCurrencySource()));
                preparedStatement.setInt(2, Integer.parseInt(currencyValue.getCurrencyDestination()));
        ...
        try (Connection connection = DBConnection.getConnection()) {
            ...
            try (Statement statement = connection.createStatement();
                ...
                while (resultSet.next()) {
                    int id = resultSet.getInt("value_id");
                    String currencySource = String.valueOf(resultSet.getInt("currency_source"));
                    String currencyDestination = String.valueOf(resultSet.getInt("currency_destination"));
    ...
    default void deleteCurrencyValue(int currencyValueId) {
        try (Connection connection = DBConnection.getConnection()) {
            String query = "DELETE FROM CurrencyValue WHERE value_id = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
